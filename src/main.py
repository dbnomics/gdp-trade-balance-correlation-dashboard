...
selected_countries = st.multiselect('Select countries to analyze', list(gdp_series.keys()),
                                    default=list(gdp_series.keys()))

# Fetch and prepare GDP data
df_gdp_all = fetch_and_prepare_data({country: gdp_series[country] for country in selected_countries},
                                    'GDP Growth Rate (%)')

# Fetch and prepare Trade Balance data
df_tb_all = fetch_and_prepare_data({country: tb_series[country] for country in selected_countries},
...
if df_gdp_all.empty or df_tb_all.empty:
    ...
    fig_tb = px.line(df_tb_all, x='Date', y='Trade Balance (% of GDP)', color='Country',
                     labels={'Date': 'Date', 'Trade Balance (% of GDP)': 'Trade Balance (% of GDP)',
                             'Country': 'Country'},
    ...
    correlations = \
        correlation_df.groupby('Country')[['GDP Growth Rate (%)', 'Trade Balance (% of GDP)']].corr().iloc[0::2][
            'Trade Balance (% of GDP)']
    ...
    st.link_button(":blue[*Pearson's coefficient definition*]",
